        ...
        call.enqueue(new Callback<User>() {
            ...
            public void onResponse(Call<User> call, Response<User> response) {
                if (response.isSuccessful()) {
                    user = response.body();
                    Toast.makeText(TelaCadastro.this, "" + response.message().toString(), Toast.LENGTH_LONG).show();
                    //FUNCAO SQLITE
//                    localDatabase(user);
                } else {
                    Log.i("RETROFIT", response.message());
                    Toast.makeText(TelaCadastro.this, "Erro: " + response.message(), Toast.LENGTH_LONG).show();
                }
            }

            @Override
            public void onFailure(Call<User> call, Throwable t) {
                Toast.makeText(TelaCadastro.this, "Algo deu errado :(" + t.getMessage(), Toast.LENGTH_LONG).show();
            }
        });
    }
